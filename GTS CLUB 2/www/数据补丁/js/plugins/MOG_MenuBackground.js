 ...
 * @desc 文件目录的位置（如果开启相册壁纸请保证和相册同位置）
 ...
 *该插件允许更改菜单背景图像。
 *
 *可以为每个场景启用不同的背景图像，或在特定场景中禁用背景图像。
 *
 *=============================================================================
 *
 *背景图像应放置在 /img/pictures/ 文件夹中
 *
 *=============================================================================
 *
 *若启用"独特背景"选项，每个背景图像需按以下规则命名：
 *
 *场景名称 + _back.png
 *
 *示例：
 ...
 * - 修复旧存档背景兼容性的问题
 *
 *v1.2 - 实现与 MOG_PictureGalery 的兼容性
 *
 *v1.1 - 代码优化改进
 */
...
Scene_MenuBase.prototype.refreshBackgroundName = function() {
	...
	this._backImg = ImageManager.picturegallery(this.set_background_img());
};
...
Scene_MenuBase.prototype.refreshBackgroundBitmap = function() {
	...
	var scaleM = Math.max(this._backgroundSpriteNew.scale.x, this._backgroundSpriteNew.scale.y)
	this._backgroundSpriteNew.scale.x = scaleM;
	this._backgroundSpriteNew.scale.y = scaleM;
...
//=============================================================================
// 修复旧存档缺少 _backgroundName 的问题
//=============================================================================
var _MOG_MB_backup_extractSaveContents = DataManager.extractSaveContents;
DataManager.extractSaveContents = function(contents) {
    _MOG_MB_backup_extractSaveContents.call(this, contents);
    if ($gameSystem && $gameSystem._backgroundName === undefined) {
        $gameSystem._backgroundName = String(Moghunter.mback_fileName);
    }
};